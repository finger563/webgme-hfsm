...
function buildMeta(projectJson, sourceLabel) {
  ...
  function resolve(metaPath, seen) {
    ...
    var mine = own[metaPath] || { attributes: {}, children: {}, pointers: {},
                                  registry: {} };
    var base = baseOf[metaPath];
    var inherited = (base && own[base])
      ? resolve(base, seen)
      : { attributes: {}, children: {}, pointers: {}, registry: {} };
  ...
  metaPaths.slice().sort(function (a, b) {
    ...
    Object.keys(rules.attributes).sort().forEach(function (attr) {
      attributes[attr] = rules.attributes[attr];
    });
...
function main(argv) {
  ...
  if (check) {
    if (!fs.existsSync(OUTPUT)) {
      console.error('gen-meta: ' + path.relative(repoRoot, OUTPUT) +
                    ' does not exist; run `npm run gen:meta`');
      return 1;
    }
    var committed = fs.readFileSync(OUTPUT, 'utf8');
    if (committed !== text) {
      console.error('gen-meta: ' + path.relative(repoRoot, OUTPUT) +
                    ' is out of sync with ' + path.relative(repoRoot, source));
      console.error('  the metamodel changed without regenerating it;' +
                    ' run `npm run gen:meta` and commit the result');
      return 1;
    }
    console.log('gen-meta: ' + path.relative(repoRoot, OUTPUT) +
                ' matches ' + path.relative(repoRoot, source) +
                ' (' + Object.keys(meta.types).length + ' types)');
    return 0;
  }

  fs.writeFileSync(OUTPUT, text);
  console.log('gen-meta: wrote ' + path.relative(repoRoot, OUTPUT) +
